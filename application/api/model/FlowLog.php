    ...
    {   
        ...
        if(isset($data['data']['status'])&&$data['data']['status']!=1){
            ...
            if($FlowLogInfo['type']==2){
                $res = UserInfo::where('user_no', $UserInfo['user_no'])->update(['balance' => $UserInfo['balance']-$FlowLogInfo['data']['count']]);
            }else if($FlowLogInfo['type']==3){
                $res = UserInfo::where('user_no', $UserInfo['user_no'])->update(['score' => $UserInfo['score']-$FlowLogInfo['data']['count']]);
            }else if($FlowLogInfo['type']==1){
                $res = UserInfo::where('user_no', $UserInfo['user_no'])->update(['score' => $UserInfo['score']-$FlowLogInfo['data']['count']]);
                WxPay::refundOrder($FlowLogInfo['order_no']);
            };
        };
